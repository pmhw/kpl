layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	function createCard(elem, linenum, item, no) {
		var line = 12 / linenum;
		var card =
			'<div id=' + item.id + ' onclick="cardTableCheckedCard(' + elem + ',this)" class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no+'" data-number="1"> <div class="project-list-item"> <img class="project-list-item-cover" src="' +item.image + '"> <div class="project-list-item-body"> <h2>' + item.title + '</h2> <div class="project-list-item-text layui-text">' + item.remark + '</div> <div class="project-list-item-desc"> <span class="time">' + '主题作者：' + item.zuozhe + '</span> <div class="ew-head-list"></div> </div> </div > </div > </div > '
		return card;
	}
	/** 格式化返回参数 */
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			item.zuozhe = dataList[i][option.request.zuozheName];
	...
	window.cardTableCheckedCard = function (elem,obj) {
		...
		//console.log(item.remark);获取主题名称
		$.get('/theme_setting',{'templates':item.remark},function(res){
		    if(res.cod>0){
	...
	var tt = {
		...
		getChecked: function (id) {
			...
			item[option.request.zuozheName] = data.zuozhe;
		...
		getAllData: function (id) {
			...
			for (var i = 0; i < option.data.length; i++) {
				...
				item[option.request.zuozheName] = option.data[i].zuozhe;
